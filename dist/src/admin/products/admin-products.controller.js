...
let AdminProductsController = class AdminProductsController {
    ...
    async createProduct(request, dto) {
        const product = await this.adminProductsService.createProduct(request.user.userId, dto);
        return (0, api_response_util_1.success)(product);
    }
    async updateProduct(request, id, dto) {
        const product = await this.adminProductsService.updateProduct(request.user.userId, id, dto);
        return (0, api_response_util_1.success)(product);
    }
    async softDeleteProduct(request, id) {
        const product = await this.adminProductsService.softDeleteProduct(request.user.userId, id);
        return (0, api_response_util_1.success)(product);
    }
};
...
__decorate([
    ...
    __metadata("design:returntype", Promise)
], AdminProductsController.prototype, "softDeleteProduct", null);
